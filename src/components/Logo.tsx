
import React, { useEffect, useState } from 'react';
import { useIsMobile } from '@/hooks/use-mobile';
import { Gamepad2 } from 'lucide-react';

/**
 * This custom logo component renders the animated Cuephoria logo.
 */
interface LogoProps {
  size?: 'sm' | 'md' | 'lg';
  className?: string;
  showText?: boolean;
}

const sizeMap = {
  sm: 32,
  md: 52,
  lg: 80,
};

const Logo: React.FC<LogoProps> = ({ size = 'md', className, showText = false }) => {
  const isMobile = useIsMobile();
  const [animate, setAnimate] = useState(false);

  // Prefer smaller logo for mobile regardless of size prop (for navbar fit)
  const height = isMobile ? 36 : sizeMap[size] || 52;
  const width = height;

  // Add animation after initial load
  useEffect(() => {
    const timer = setTimeout(() => {
      setAnimate(true);
    }, 300);
    
    return () => clearTimeout(timer);
  }, []);

  return (
    <div className={`flex items-center gap-3 ${className || ""}`}>
      <div 
        className={`flex items-center justify-center rounded-full bg-gradient-to-r from-[#9b87f5] to-[#D6BCFA] shadow-lg 
                    ${animate ? 'animate-pulse-soft' : ''} transition-all duration-500`}
        style={{ 
          height, 
          width,
          boxShadow: animate ? '0 0 20px rgba(155, 135, 245, 0.4)' : 'none' 
        }}
      >
        <Gamepad2 className={`text-white ${animate ? 'animate-scale-in' : ''}`} 
                   size={Math.round(height * 0.6)} />
      </div>
      
      {showText && (
        <span className={`font-bold text-[#9b87f5] whitespace-nowrap 
                          ${animate ? 'animate-fade-in' : 'opacity-0'} 
                          ${size === 'sm' ? 'text-xl' : size === 'lg' ? 'text-3xl' : 'text-2xl'}`}>
          Cuephoria
        </span>
      )}
    </div>
  );
};

export default Logo;
