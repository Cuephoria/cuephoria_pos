        ...
        <div className="flex items-center text-lg font-heading">
          {isPoolTable ? (
            <div className="relative w-10 h-10 flex items-center justify-center">
              <div className="absolute inset-0 bg-gradient-to-br from-blue-900/50 to-blue-950/50 backdrop-blur-md rounded-md"></div>
              <CircleOff className="h-6 w-6 text-blue-300 z-10 opacity-80" />
              <div className="absolute inset-0 border-2 border-blue-700/50 rounded-md"></div>
            </div>
          ) : (
            <div className="relative w-10 h-10 flex items-center justify-center">
          ...
          <span className={`ml-2 font-bold ${isPoolTable ? 'text-blue-400' : 'text-cuephoria-lightpurple'}`}>
            {station.name}
          </span>
        </div>
        <Badge 
