...
const CustomerProtectedRoute = ({ children }: CustomerProtectedRouteProps) => {
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-cuephoria-dark">
      <div className="animate-spin-slow h-10 w-10 rounded-full border-4 border-cuephoria-orange border-t-transparent"></div>
    </div>;
  }
  
  if (!customerUser) {
    // Redirect to customer login page while preserving the intended destination
